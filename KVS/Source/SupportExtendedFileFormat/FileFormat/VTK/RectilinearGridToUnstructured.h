 ...
 */
#ifndef EXTENDED_FILE_FORMAT_RECTILINEAR_GRID_TO_UNSTRUCTURED_H_INCLUDE
#define EXTENDED_FILE_FORMAT_RECTILINEAR_GRID_TO_UNSTRUCTURED_H_INCLUDE
...
inline AxisDirection ValidateRectilinearAxis( const std::vector<double>& coordinates,
...
/**
 * @brief 直交座標軸と点データから線形六面体の非構造格子を生成する。
 */
inline vtkSmartPointer<vtkUnstructuredGrid> RectilinearGridToLinearHexahedra(
    const std::vector<double>& x_coordinates, const std::vector<double>& y_coordinates,
    const std::vector<double>& z_coordinates, vtkDataSet* source_data )
{
    ...
    {
        throw std::invalid_argument( "point-data tuple count does not match nx * ny * nz" );
    }
...
/**
 * @brief VTK直交格子の座標配列と点データから線形六面体格子を生成する。
 */
inline vtkSmartPointer<vtkUnstructuredGrid> RectilinearGridToLinearHexahedra(
...
#endif // EXTENDED_FILE_FORMAT_RECTILINEAR_GRID_TO_UNSTRUCTURED_H_INCLUDE
